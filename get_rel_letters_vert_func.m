function get_rel_letters_vert_func(input_imagename)
close all
% clear all;
clc;
...
all_y2 = cell2mat(cellfun(@(x) x(4), bboxes,'UniformOutput',false));
all_h = all_y2 - all_y1;
dy_distmat = sqrt(sp_dist2(all_y1,all_y1));
...
y1 = boxes(:,2);
y2 = boxes(:,4);
y1_mat1 = repmat(y1,1,nnodes);
y2_mat2 = repmat(y2',nnodes,1);
b2bdist = abs(y1_mat1 - y2_mat2);
...
%         inline0_relmat(inlin_letters(j),inlin_letters(j+1)) = 1;
        inline90_relmat(inlin_letters(j),inlin_letters(j+1)) = 1;
    end
end
...
%     cntr_distmat_divw(i,:) = cntr_distmat(i,:)/w;
    cntr_distmat_divh(i,:) = cntr_distmat(i,:)/h;
end
...
%         n2_y2 = node2_box(4);
        ...
        min_x1 = min(n1_x1,n2_x1);
        max_x1 = max(n1_x1,n2_x1);
        min_x2 = min(n1_x2,n2_x2);
        max_x2 = max(n1_x2,n2_x2);
        
        union = max_x2 - min_x1;
        intersect  = min_x2 - max_x1;
...
h1_mat = repmat(all_h,1,nnodes);
h2_mat = repmat(all_h',nnodes,1);
avg_h1h2 = (h1_mat + h2_mat)/2;
max_h1h2 = max(cat(3,h1_mat,h2_mat),[],3);
sum_h1h2 = sum(cat(3,h1_mat,h2_mat),3);
...
% connectivity_and = (ov_conctvals_mat>=ov_thresh ).*(0<=(1.5*max_h1h2 - dy_distmat)).*(ov_mat <=0.2).*(0<=(avg_h1h2/2 - b2bdist)).*...
%                    (area12_mat>=are_lowthresh).*(area12_mat<=are_highthresh).*...
%                    (0<=(clr_threshold - clr_distmat_norm));

connectivity_and = (ov_conctvals_mat>=ov_thresh ).*(0<=(2*max_h1h2 - dy_distmat)).*(ov_mat <=0.2).*(0<=(avg_h1h2 - b2bdist)).*...
                   (area12_mat>=are_lowthresh).*(area12_mat<=are_highthresh).*...
                   (0<=(clr_threshold - clr_distmat_norm));
...
save(fullfile(data_dir,[testname '_cost_connectivity_vert.mat']),'cost','connectivity_and','nodes',...
        'clr_distmat_norm','cntr_distmat_norm','all_h','ov_conctvals_mat','inline90_relmat',...
